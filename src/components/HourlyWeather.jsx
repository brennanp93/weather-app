...
export default function HourlyWeather({ hourlyData, temp }) {
  ...
  let hourlyTemp = hourlyData?.hourly?.temperature_2m?.map(
    (t) => (t = Math.round(t))
  );
  let timeArray = hourlyData?.hourly?.time?.map(
    (t) => (t = moment(t * 1000).format("h a"))
  );
  let shortTimeData = hourlyData?.hourly?.time?.map(
    (t) => (t = new Date(t * 1000).getHours())
  );
